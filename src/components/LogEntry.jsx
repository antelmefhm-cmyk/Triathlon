...
export default function LogEntry({ onSaved }) {
  ...
  const pace = (() => {
    ...
    if (discipline === 'swim') {
      const per100m = durationSec / (Number(distanceKm) * 10);
      const m = Math.floor(per100m / 60);
      const s = Math.round(per100m % 60);
      return `${m}:${String(s).padStart(2, '0')} / 100m`;
    }
    if (discipline === 'run') {
      const perKm = durationSec / Number(distanceKm);
      const m = Math.floor(perKm / 60);
      const s = Math.round(perKm % 60);
      return `${m}:${String(s).padStart(2, '0')} / km`;
    }
    if (discipline === 'bike') {
      const kph = Number(distanceKm) / (durationSec / 3600);
  ...
  return (
    <div className="screen">
      ...
      <div className="card">
        <Field label={discipline === 'brick' ? 'Total distance (km, optional)' : 'Distance (km)'}>
          <input
            style={inputStyle}
            inputMode="decimal"
            placeholder="e.g. 5.0"
